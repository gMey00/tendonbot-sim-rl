...
\begin{figure}[ht!]
\centering
\includegraphics[width=0.55\textwidth]{rl_agent_environment_loop}
\caption[RL agent--environment interaction]{The reinforcement learning interaction loop: the agent selects action~$a_t$ from policy~$\pi$, the environment transitions and returns observation~$o_{t+1}$ and reward~$r_t$.}\label{fig:rl_agent_environment_loop}
\end{figure}
...
\subsection{Learning objectives and algorithm families}
...
\subsection{Exploration and data collection in manipulation}
...
In manipulation, exploration is complicated by long horizons, sparse success signals, and irreversible or
...
Common strategies include \textbf{domain randomization}, where visual and physical parameters are randomized during training so that
the policy learns behaviors that are robust across a range of plausible conditions~\cite{Tobin2017DomainRand}, and
\textbf{dynamics randomization}, which targets uncertainties in physical parameters (e.g., mass,
friction, motor gains, delays) to improve transfer for control policies~\cite{Peng2018DynamicsRand}.
% NOTE: A comprehensive treatment of sim-to-real transfer strategies
% (including teacher-student distillation, system identification, and
% ROS 2 integration) is provided in the Master Thesis background §2.6.
...
from unstable updates. This is a practical consideration in contact-rich manipulation where learning can be sensitive to abrupt
policy changes~\cite{Schulman2017PPO,Kober2013Survey}.

\subsection{Why PPO is often used for robotic manipulation}
...
% NOTE: The thesis-specific PPO/skrl implementation description has been
% moved to Methodology §4.4 (Reinforcement Learning Setup) because it
% describes the concrete algorithm and library choices for this project,
% not general background theory.
