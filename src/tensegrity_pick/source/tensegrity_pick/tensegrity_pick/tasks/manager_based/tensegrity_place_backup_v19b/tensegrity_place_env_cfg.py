...
FINGER_PAD_BODIES = ["left_inner_finger", "right_inner_finger"]
...
class RewardsCfg:
    ...
    Structure: reach → lift → goal tracking (coarse + fine) + light regularisation.
    Total: 7 functional terms + 2 metrics.  All rewards are bounded (tanh/binary).
    """

    # ── 1. Reach: tanh proximity at two scales for long-range gradient ─
    reaching_coarse = RewTerm(
        func=task_rew.grasp_proximity_tanh,
        weight=1.0,
        params={
            "ee_cfg": SceneEntityCfg("robot", body_names=GRASP_BODIES),
            "green_name": "green_cube",
    ...
    # ── 2. Gripper phase: open-approach then close-grasp ────────────
    gripper_phase = RewTerm(
        func=task_rew.phase_aware_gripper_reward,
        weight=5.0,
        params={
            "ee_cfg": SceneEntityCfg("robot", body_names=GRASP_BODIES),
            "green_name": "green_cube",
            "transition_distance": 0.10,
            "transition_sharpness": 30.0,
            "std": 0.15,
        },
    )

    # ── 2b. Grasp bridge: finger closure near cube ───────────────────
    # proximity_threshold must overlap with the reaching_medium sweet
    # spot (~0.17 m avg distance) to provide gradient signal.
    finger_grasp = RewTerm(
        func=task_rew.finger_grasp_reward,
        weight=8.0,
        params={
            "ee_cfg": SceneEntityCfg("robot", body_names=GRASP_BODIES),
            "green_name": "green_cube",
    ...
    # ── 3. Lift: binary bonus when cube is lifted ─────────────────────
    lifting_object = RewTerm(
        func=task_rew.green_lifted_bonus,
        weight=15.0,
        params={
            "ee_cfg": SceneEntityCfg("robot", body_names=GRASP_BODIES),
            "green_name": "green_cube",
    ...
    action_rate = RewTerm(func=task_rew.action_rate_l2, weight=-1e-4)
    joint_vel = RewTerm(
    ...
    # ── Belt contact: metric-only (for TensorBoard monitoring) ───────
    belt_contact = RewTerm(
        func=task_rew.belt_contact_penalty,
        weight=0.01,
        params={
            "asset_cfg": SceneEntityCfg("robot", body_names=FINGER_PAD_BODIES),
            "belt_height": CONVEYOR_SURFACE_HEIGHT_M,
            "margin": 0.0,
            "max_depth": 0.15,
        },
    ...
    # Provides continuous gradient to steer the policy away from
    # configurations that saturate actuators (i.e. would break the arm).
