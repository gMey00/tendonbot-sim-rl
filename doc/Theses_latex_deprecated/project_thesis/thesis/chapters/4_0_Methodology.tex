...
Each of the arm's three joints is modeled in IsaacSim as a revolute articulation with a position/force drive to emulate the tendon actuation. In the physical design, these joints would be driven by tendons via remote actuators, but in simulation they are controlled directly by setting joint angle targets and applying spring-damper forces. The first joint (shoulder/elbow pitch) allows approximately $140^\circ$ of rotation ($\pm 70^\circ$, i.e.\ $\pm 1.2217\,\mathrm{rad}$, from neutral), corresponding to the practical measured workspace of the ``elbow'' in the vertical plane as reported by Klein~\cite{Klein2023} (§4.2, p.\,79). The wrist mechanism of the arm is implemented as two consecutive revolute joints (perpendicular axes) representing the wrist's flexion/extension and abduction/adduction; each of these has a range of $\pm 50^\circ$ ($\pm 0.8727\,\mathrm{rad}$). These ranges are based on the practical measured workspace reported in the original design~\cite{Klein2023}. For simulation stability, hard joint limits are imposed in the \ac{URDF}/\ac{USD} definition to prevent the solver from pushing the joints beyond their intended travel.
...
\begin{table}[htb]
    ...
    Arm Joint 1   & Revolute (shoulder/elbow pitch) & $-70^\circ$ to $+70^\circ$ ($\pm 1.2217\,\mathrm{rad}$) \\
    Arm Joint 2   & Revolute (wrist tilt X-axis)    & $-50^\circ$ to $+50^\circ$ ($\pm 0.8727\,\mathrm{rad}$) \\
    Arm Joint 3   & Revolute (wrist tilt Y-axis)    & $-50^\circ$ to $+50^\circ$ ($\pm 0.8727\,\mathrm{rad}$) \\
    \hline
    \end{tabular}
\end{table}
...
\input{chapters/methodology/4_4_RLSetup}
