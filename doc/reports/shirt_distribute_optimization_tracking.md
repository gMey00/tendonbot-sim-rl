...
### Iteration-5 verdict: EMA-to-limits FAILS on this task

| Run | stochastic | deterministic |
|---|---|---|
| `sd_train7e` s1 (EMA, swept bank, 64 k @ walltime) | 0.82 | **0.010 / 0.010** (seeds 7/8), mean\|act\| **1.35 / 1.42** |
| `sd_train7e` s2 (80 k) | 0.54 | pending |
| `sd_train8eb` s1 (EMA, FIXED bank, 76 k) | 0.51 | pending |
| `sd_train8eb` s2 | **collapsed** (release 0.04, success 0) | — |

The EMA mean DIFFUSES beyond the valid action band (|a| > 1 all clamp to the
same limit pose ⇒ zero gradient out there; `action_l2` was zeroed for this
variant) — the saturation disease reappears in target space, σ anneals even
slower (0.58 → 0.54 at 64 k), and one seed per array collapses.  The sibling
task's EMA win does not transfer: shirt_present is quasi-static reaching;
this task needs a TIMED release, where EMA's smoothing and the free mean
diffusion hurt.  **Verdict: stay with relative actions + `action_l2` (the
measured best: deterministic 0.56 on eval seed 8 — confounded by the init
shift that is now fixed).**

### Iteration 6 (`sd_train9fb`, seeds 1/2/3, 96 k, FIXED bank)

The strongest measured recipe — relative actions + `action_l2` curriculum +
`settle_after_success` + true belt termination + `bad_release` discovery
curriculum — trained ON the cached pose bank, so training and deterministic
evaluation share one init distribution for the first time, at the doubled
budget the resume experiment showed still helps.
