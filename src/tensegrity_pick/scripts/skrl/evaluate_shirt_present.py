"""Deterministic evaluation of shirt_present checkpoints (mean actions).
...
metrics (present/grasp/drop rate + final coverage/stretch) over exactly the
envs that just finished; this script accumulates those batch means weighted
by batch size until ``--num_episodes`` episodes are in.

Prints: present_rate (windowed stretched-hold latch), grasp_rate, drop_rate,
final coverage / stretch ratio, mean |action| (saturated-noise indicator,
shirt_place lesson) and a final one-line RESULT.

Usage::

    cd src/tensegrity_pick
    conda activate env_isaaclab
    PYTHONUNBUFFERED=1 python scripts/skrl/evaluate_shirt_present.py --headless \
        --task Template-Shirt-Present-UR5e-F140-v0 --num_envs 32 --seed 7 \
        --num_episodes 96 \
        --checkpoint logs/skrl/shirt_present/<run>/checkpoints/agent_20000.pt

    # stochastic actions instead of the mean (determinism-gap check):
    PLAY_STOCH=1 python scripts/skrl/evaluate_shirt_present.py ...
"""
...
parser = argparse.ArgumentParser(description="Deterministic shirt_present checkpoint evaluation.")
...
def main() -> None:
    ...
    # Seed the global torch RNG explicitly: the hanging-bank sampling /
    # augmentation in the env reset uses it, and cfg.seed alone was observed
    # NOT to differentiate eval rollouts (shirt_pick eval lesson).
    ...
    while simulation_app.is_running() and episodes < args_cli.num_episodes:
        ...
        if done > 0:
            ...
            print(f"[eval] episodes {episodes}/{args_cli.num_episodes}: "
                  ...
                  f"drop {acc['drop']/episodes:.3f} "
                  f"cov {acc['cov']/episodes:.3f} "
                  f"stretch {acc['stretch']/episodes:.3f}", flush=True)

    mode = "stochastic" if stochastic else "deterministic"
    e = max(episodes, 1)
    print(f"\nRESULT ({mode}, seed {args_cli.seed}, {episodes} episodes): "
          f"present_rate={acc['present']/e:.3f} "
          f"grasp_rate={acc['grasp']/e:.3f} "
          f"drop_rate={acc['drop']/e:.3f} "
          f"final_coverage={acc['cov']/e:.3f} "
          f"final_stretch={acc['stretch']/e:.3f} "
