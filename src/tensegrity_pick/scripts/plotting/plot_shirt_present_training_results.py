#!/usr/bin/env python3
"""Generate training result plots for the Shirt Present task README.
...
``scripts/model_validation/read_tfevents.py``) — port of
``plot_shirt_pick_training_results.py``.

Usage
-----
    cd src/tensegrity_pick
    python scripts/plotting/plot_shirt_present_training_results.py [--run DIR_NAME] [--label NAME]

If ``--run`` is omitted the latest ``*_ppo_torch*`` run under
``logs/skrl/shirt_present/`` is used.  ``--label`` selects the output
...
LOGS_ROOT: Final = Path("logs/skrl/shirt_present")
FIGURES_BASE: Final = Path(
    "source/tensegrity_pick/tensegrity_pick/tasks/manager_based/shirt_present/figures"
)
...
def plot_total_reward(series: dict, out: Path) -> None:
    ...
    ax.set_title("Shirt Present — Total Episode Reward"); ax.legend()
...
def plot_task_metrics(series: dict, out: Path) -> None:
    ...
    for tag, label, colour in [
        ("Metrics/grasp_rate", "second-grasp rate", _C_GREEN),
        ("Metrics/present_rate", "present rate (windowed stretched hold)", _C_BLUE),
    ...
    ax.set_title("Shirt Present — Task Metrics (stochastic rollouts; deterministic eval differs)")
    ax.legend()
    finalise(fig, out / "02_task_metrics.png")


def plot_cloth_metrics(series: dict, out: Path) -> None:
    fig, ax = plt.subplots(figsize=FIGSIZE_WIDE)
    for tag, label, colour in [
        ("Metrics/final_coverage", "final silhouette coverage", _C_TEAL),
        ("Metrics/final_stretch_ratio", "final stretch ratio", _C_PURPLE),
    ]:
        s, v = get(series, tag)
        if s.size:
            ax.plot(k(s), v, color=colour, alpha=0.2)
            ax.plot(k(s), smooth(v), color=colour, label=label)
    mark_curriculum(ax)
    ax.set_xlabel("trainer steps (k)"); ax.set_ylabel("value at episode end")
    ax.set_title("Shirt Present — Cloth Metrics (coverage & tautness)")
    ax.legend()
    finalise(fig, out / "03_cloth_metrics.png")


def plot_reward_decomposition(series: dict, out: Path) -> None:
    terms = [
        ("Episode_Reward/reaching", "reach (tip → lowest point)", _C_BLUE),
        ("Episode_Reward/grasp_hold", "grasp hold", _C_GREEN),
        ("Episode_Reward/stretch", "stretch progress", _C_TEAL),
        ("Episode_Reward/coverage", "coverage (gated)", _C_PURPLE),
        ("Episode_Reward/presented", "presented (success, per-step)", _C_ORANGE),
        ("Episode_Reward/overstretch", "overstretch (penalty)", _C_AMBER),
    ...
    axes[0].set_title("Shirt Present — Reward Decomposition (sequential skill acquisition)")
...
def plot_penalties(series: dict, out: Path) -> None:
    ...
    mark_curriculum(ax)
    ax.set_xlabel("trainer steps (k)"); ax.set_ylabel("episode reward")
    ax.set_title("Shirt Present — Penalties & Regularisation"); ax.legend()
...
def plot_episode_length(series: dict, out: Path) -> None:
    ...
    ax.set_title("Shirt Present — Episode Length"); ax.legend()
...
def main() -> None:
    ...
    parser.add_argument("--run", type=str, default=None,
                        help="run directory name under logs/skrl/shirt_present/")
