...
Drop-in replacement for the tensegrity place task using a Kinova Gen3 (7-DOF)
+ Robotiq 2F-140.  Sequential reward structure is identical to enable fair
comparison (PG-6: Tendon Wrist Transfer Experiment).

Key differences from the tensegrity variant:
  - 7-DOF revolute arm (no prismatic base) → single JointPositionActionCfg
  - No base_velocity penalty (fixed-base robot)
  - arm_utilization covers all 7 arm joints
"""
...
from . import mdp
from ..shared.proj_base_scene_cfg import KINOVA_MOUNT_HEIGHT_M
from .place_scene_cfg import (
    PlaceSceneCfg,
...
@configclass
class KinovaPlaceSceneCfg(PlaceSceneCfg):
    """Place scene with Kinova Gen3 + Robotiq 2F-140 instead of tensegrity."""

    robot = KINOVA_GEN3_GRIPPER_CFG.replace(
        prim_path="{ENV_REGEX_NS}/Robot",
        init_state=ArticulationCfg.InitialStateCfg(
            pos=(0.15, 0.0, KINOVA_MOUNT_HEIGHT_M),
            # 180° about Z to flip upside-down for ceiling mount.
            rot=(0.0, 0.0, 1.0, 0.0),
...
@configclass
class KinovaPlaceActionsCfg:
    """Joint-position delta actions (7 arm joints + binary gripper)."""
...
@configclass
class KinovaPlaceObservationsCfg:
    """Observation specification — Kinova controlled joints."""
...
@configclass
class KinovaEventsCfg:
    """Reset events for the Kinova place task."""
    ...
    reset_arm = EventTerm(
        ...
        mode="reset",
        params={
            "asset_cfg": SceneEntityCfg("robot", joint_names=KINOVA_ARM_JOINT_NAMES),
...
@configclass
class KinovaRewardsCfg:
    """Reward terms — identical structure/weights to tensegrity, Kinova joints.
    ...
    # No base_velocity penalty — Kinova is a fixed-base robot.
...
@configclass
class KinovaPlaceEnvCfg(ManagerBasedRLEnvCfg):
    """Kinova Gen3 cube-placement task — fair comparison baseline."""

    scene: KinovaPlaceSceneCfg = KinovaPlaceSceneCfg(num_envs=8192, env_spacing=5.0)
    actions: KinovaPlaceActionsCfg = KinovaPlaceActionsCfg()
    observations: KinovaPlaceObservationsCfg = KinovaPlaceObservationsCfg()
    terminations: KinovaTerminationsCfg = KinovaTerminationsCfg()
    rewards: KinovaRewardsCfg = KinovaRewardsCfg()
    events: KinovaEventsCfg = KinovaEventsCfg()
    curriculum: KinovaCurriculumCfg = KinovaCurriculumCfg()
