...
from ..shared.gripper_cfg import SpawnBox
...
# ── Randomized cloth placement on the belt ─────────────────────────────────
# At each reset the shirt is laid flat, randomly translated within this box
# (local env coords, X/Y only — Z is set so the shirt rests on the belt) and
# rotated by a random yaw.  With probability ``SHIRT_FOLD_PROB`` it is
# additionally half-folded for initial-state diversity.
SHIRT_SPAWN_BOX = SpawnBox(
    x_range=(0.05, 0.20),
    y_range=(-0.12, 0.12),
    z_range=(0.0, 0.0),  # unused — Z derived from belt height
    yaw_range=(-3.14159, 3.14159),
)
SHIRT_BELT_CLEARANCE_M = 0.02  # drop height above the belt before settling
SHIRT_FOLD_PROB = 0.3          # fraction of resets that start half-folded
