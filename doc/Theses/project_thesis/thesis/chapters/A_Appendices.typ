...
To observe training progress in real time, a custom monitoring tool was developed as part of this work. Reinforcement learning experiments in Isaac Lab using the SKRL library produce TensorBoard event files containing all logged scalar metrics. While TensorBoard itself provides a web-based visualization, it requires a graphical browser and does not present task-specific summaries such as training speed, estimated time of arrival, or reward decomposition in a consolidated view. Since the training workstation is accessed exclusively over SSH, a terminal-native solution was needed.

The monitor is implemented as a Python package (`tools.monitor`) with four modules. The *data layer* (`core.py`) handles run discovery and TensorBoard parsing. It recursively scans the log directories for `run_status.json` files that the training scripts write at startup, and uses the `tbparse` library to read scalar events into a Pandas DataFrame. All metric tag names are auto-discovered on every refresh cycle — no metric names are hardcoded. Tags are categorized by their prefix (e.g.\ `Episode_Reward/`, `Loss/`, `Metrics/`) so the dashboard can group them semantically. The module also computes training speed in steps per second and estimates the remaining time from wall-clock timestamps.

The *terminal frontend* (`terminal.py`) builds a Rich Live dashboard rendered directly in the terminal. It uses `plotext` to draw inline line charts of reward curves and policy diagnostics using Unicode block characters. Three switchable views are provided — *Overview*, *Reward Charts*, and *Policy* — navigable with the number keys 1–3. A background thread reads raw keypresses via `termios` to allow view switching without interrupting the Rich render loop. @fig:monitor_tui_overview shows the overview with tabular reward terms, learning metrics, and custom task metrics. @fig:monitor_tui_rewards shows the reward chart view with total reward (mean/max/min) and the top reward term decomposition. @fig:monitor_tui_policy shows the policy diagnostics view with standard deviation, losses, and learning rate.
...
#faps-figure(
  image("../../../shared/figures/monitor_tui_overview.png", width: 100%),
...
#faps-figure(
  image("../../../shared/figures/monitor_tui_rewards.png", width: 100%),
...
#faps-figure(
  image("../../../shared/figures/monitor_tui_policy.png", width: 100%),
...
#faps-figure(
  image("../../../shared/figures/monitor_webapp.png", width: 100%),
