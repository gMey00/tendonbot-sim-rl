...
Automating textile sorting with robotic systems is a promising approach to increasing throughput and consistency, but it presents significant technical challenges. Garments are deformable objects that exhibit highly variable shapes, complex self-interactions, and contact dynamics that differ fundamentally from the rigid-object manipulation scenarios for which most industrial robots are designed~@Longhini2025ReviewClothManipulation. Reliable textile sorting requires not only picking individual items from a mixed waste stream but also manipulating them into configurations that allow visual inspection---for example, stretching a garment flat to expose damage---and then routing each item to the correct output bin based on the inspection result. This sequence of pick, inspect, and sort naturally involves multiple manipulation primitives and can benefit from coordination between multiple robot arms operating on the same workpiece.

#ac("RL") offers a data-driven approach to learning such complex manipulation behaviors. Rather than manually designing controllers for each manipulation primitive, #ac("RL") enables policies to be discovered through trial-and-error interaction with the task environment, guided by a reward signal that encodes the desired outcome~@SuttonBarto2018 @Kober2013Survey. When extended to _multi-agent_ settings, #ac("RL") can learn coordinated policies for teams of robots that must share a workspace, hand off workpieces, and jointly optimize a system-level objective.

Modern #ac("GPU")-accelerated physics simulators have become essential enablers for this approach. By running thousands of environment replicas in parallel, they provide the sample throughput required for stable policy learning while avoiding the cost and risk of training on physical hardware~@Makoviychuk2021IsaacGym. NVIDIA Isaac~Sim, with its PhysX~5 physics backend and the associated IsaacLab framework, provides an integrated toolchain that combines high-fidelity rigid-body and deformable-object simulation with a modular #ac("MDP")-based task interface suitable for large-scale #ac("RL") training~@nvidia_isaac_sim_510_what_is @Mittal2025IsaacLab. Recent developments in the Newton physics engine further extend this capability to realistic cloth simulation using Vertex Block Descent (#ac("VBD"))~@chen2024vbd @nvidia_newton_announcement_2025, making simulation-based development of cloth manipulation policies increasingly practical.

This thesis is the second part of a two-part project conducted at the Institute for Factory Automation and Production Systems (FAPS) at Friedrich-Alexander-Universität Erlangen-Nürnberg. The preceding project thesis (Meyer, 2026) established a validated simulation of the 5-#ac("DoF") tendon-driven robot and demonstrated #ac("RL")-based control for reach and rigid-object place tasks in Isaac~Sim. The present work extends that foundation to the full textile sorting pipeline: integrating deformable cloth simulation, adding a second collaborative robot arm, implementing camera-based damage classification, training coordinated multi-agent policies, and preparing the system for sim-to-real transfer.

== Problem Statement and Objectives

Condition-based textile sorting from mixed waste streams poses a multi-faceted robotic manipulation problem that goes substantially beyond standard pick-and-place. The task requires a system that can (i)~selectively retrieve target garments (e.g., T-shirts) from a moving conveyor while ignoring non-target objects, (ii)~present each garment in a configuration suitable for visual inspection (e.g., stretched flat), (iii)~classify the garment's condition through camera-based analysis, and (iv)~route the item to the appropriate output bin. Each of these sub-tasks involves contact-rich interaction with deformable objects whose behavior is difficult to model analytically~@Longhini2025ReviewClothManipulation, and the full pipeline requires coordination between multiple robotic agents operating in a shared workspace.

The robotic system considered in this thesis consists of two manipulators. The first is the tendon-driven 5-#ac("DoF") pick robot validated in the project thesis: a 2-#ac("DoF") prismatic positioning stage, a 3-#ac("DoF") tendon-driven arm, and a Robotiq 2F-140 gripper, based on the design by Klein~@Klein2023. The second is a collaborative sorting arm that receives the garment from the pick robot, stretches it for camera-based inspection, and deposits it into a condition-specific bin. The two robots must coordinate their actions---timing the handoff, avoiding collisions, and jointly optimizing the sorting throughput and accuracy---which naturally frames the problem as a multi-agent #ac("RL") task.

A critical technical challenge is the simulation of cloth dynamics with sufficient fidelity for policy learning. Deformable cloth introduces a high-dimensional state space (many particles/vertices), complex self-collisions and folding behavior, and sensitivity to material parameters such as bending stiffness, friction, and damping~@Longhini2025ReviewClothManipulation @muller2007pbd. Policies trained on inaccurate cloth models risk exploiting simulation artifacts that do not transfer to real textiles, making the _reality gap_ a central concern~@Salvato2021RealityGapSurvey. Domain randomization over cloth and environment parameters is therefore essential for developing robust policies~@Tobin2017DomainRandomization @Peng2018DynamicsRandomization.

The thesis pursues the following objectives:

+ *Conveyor-based sorting task---single agent (PG-1):*
...
+ *Cloth simulation integration (PG-2):*
  Integrate deformable cloth simulation into the Isaac~Sim environment using PhysX particle cloth (#ac("PBD")) or Newton thin deformables (#ac("VBD"))~@muller2007pbd @chen2024vbd. Model T-shirt objects with realistic material properties and validate cloth behavior through qualitative and quantitative benchmarks. Apply domain randomization over cloth parameters to support robust policy learning.

+ *Second robot arm integration (PG-3):*
...
+ *Camera-based damage classification (PG-4):*
...
+ *Sim-to-real transfer preparation (PG-6):*
  Assess transfer robustness through systematic domain randomization (conveyor speed, cloth properties, sensor noise, lighting)~@Tobin2017DomainRandomization @Peng2018DynamicsRandomization. Define the #ac("ROS")~2 integration architecture for physical deployment: joint state publishers, policy inference nodes, camera pipelines, and safety constraints. Identify and document remaining gaps for real-world operation.

+ *Evaluation and benchmarking (PG-7):*
...
This thesis is structured as follows. @ch:background provides the background required for this work, beginning with a summary of the foundation established in the project thesis (tendon-driven robot model, simulation environment, and #ac("RL") pipeline) and then covering the additional topics central to the master thesis: cloth and deformable object simulation, multi-agent #ac("RL"), vision-based classification, prior work on robotic cloth manipulation and sorting, and sim-to-real transfer strategies.

@ch:research_gap identifies the specific research gap: the absence of an integrated, simulation-based multi-agent system that combines tendon-driven manipulation, deformable cloth handling, camera-based condition classification, and coordinated sorting within a single #ac("RL")-trainable environment.

@ch:methodology describes the methodological approach, including the cloth simulation setup, the second robot arm integration, the damage classification pipeline, the multi-agent #ac("MDP") formulation, the #ac("RL") training procedures, and the domain randomization strategy for sim-to-real robustness.

@ch:results presents the experimental results across all objectives: single-agent sorting performance, cloth simulation fidelity, multi-agent coordination metrics, classification accuracy, and the end-to-end sorting evaluation.

@ch:discussion discusses the implications and limitations of the approach, including cloth simulation fidelity and its effect on policy transfer, the effectiveness of multi-agent coordination versus sequential single-agent execution, and the remaining challenges for real-world deployment.

@ch:conclusion concludes the thesis, summarizes the contributions, and identifies directions for future work, including physical deployment via the #ac("ROS")~2 integration pathway.
