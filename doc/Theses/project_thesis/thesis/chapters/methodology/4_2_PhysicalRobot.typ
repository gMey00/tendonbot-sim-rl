...
which grows monotonically toward the workspace boundary; the FAU elbow inherits the same qualitative error budget from the disc approximation.

Two further geometric consequences bound the usable workspace. First, the antiparallelogram has a parallelogram-bifurcation singularity when the two crossed side links become collinear with the frame --- nominally at $phi = plus.minus 90 degree$ --- so the empirical $plus.minus 75 degree$ limit reported by Klein~@Klein2023 keeps a $approx 15 degree$ margin from this kinematic singularity. Second, the input-to-output relation $phi(theta)$ derived from the closure condition (@eq:loop_closure) is non-linear: a uniform sweep of the elbow output through $plus.minus 75 degree$ corresponds to an asymmetric rod-deviation interval of approximately $[-21 degree, +35 degree]$ around $theta_0$, which is what the simulation step-response tests in @sec:model_validation actually exercise.

==== Tendon actuation of the elbow
Two antagonistic tendons are attached at the midpoints of the side links and route via Bowden sheaths to the ceiling-mounted Maxon EC60 spool drums (cf.~@subsec:drive_system); the same Bowden-cable-with-#ac("APM") topology forms the design template adopted by Wang et al.~@Wang2023humanoid and reused by Klein~@Klein2023. When one tendon is tensioned, the crossed side links rotate, the coupler (forearm) swings about the migrating #ac("ICR") of @eq:centrode_axes, and the second tendon pays out under residual pre-tension. The elbow torque is therefore a function of both the applied cable tension and the instantaneous linkage geometry. For the disc-approximation variant this relationship is linearised through the constant lever arm $r_e = 72.5 "mm"$, yielding the antagonistic torque envelope reported in @eq:effort_elbow; for the physical variant the torque is recomputed at every step from the actual force-application geometry of the current configuration.
