...
# task 2 start).  Constraints (empirically checked by
# scripts/model_validation/baseline_shirt_pick.py — the earlier (0.15, 0.45,
# 1.25) was OUTSIDE the tensegrity carry envelope):
#   * z ≤ ~1.15: grasp-point carry max ≈ 1.2 m (shirt_place kinematics);
#   * y ≥ 0.5:  the hanging shirt (~0.35 m below the grasp) must clear the
#     belt footprint (collider y-extent ±0.45) so it hangs free for task 2;
#   * near the inspection camera's −Y optical axis (camera at y=2.0, z=1.25).
PRESENTATION_POS = (0.15, 0.50, 1.10)
