...
This chapter describes the complete workflow for constructing, validating, and using a tendon-driven tensegrity manipulator in simulation for #ac("RL")-based manipulation tasks. The presentation follows the temporal sequence of the project: first, the robot design — combining the FAPS-built tensegrity arm with simulated base and gripper components — is introduced (@sec:physical_robot). Next, the process for translating the #ac("CAD") geometry into a physics-ready simulation model is detailed (@sec:sim_model_construction). The tendon actuation layer that maps cable tensions to joint torques in simulation is then derived (@sec:tendon_actuation). Validation of the resulting model against reference data is presented (@sec:model_validation). Finally, the formulation of #ac("RL") tasks built on top of this model is described (@sec:rl_tasks), together with the training infrastructure (@sec:training_infrastructure).
