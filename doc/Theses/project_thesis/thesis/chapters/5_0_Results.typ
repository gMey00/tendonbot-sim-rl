...
#todo[Several reference values are missing from the source records and are plotted as hatched "n/a" placeholders. Fill in: (i)~Klein~(2023) rise time, settling time and overshoot per joint; (ii)~PD and constant-tension tendon overshoot for elbow and wrist; (iii)~physical-tendon wrist-X / wrist-Y rise- and settling-time means recorded from the simulation. After updating `assets/data/step_response/validation_metrics.json` and the `PHYSICAL_MEANS` dict in `scripts/plot_step_metrics.py`, run `make step-metrics` from `project_thesis/scripts/` to regenerate.]

#faps-figure(
  image("../../assets/figures/results/step_metrics_overview.png", width: 100%),
  caption: [Cross-variant comparison of step-response performance metrics, aggregated as the mean across the per-joint amplitude sweep. Each panel compares the Klein~(2023) Gazebo reference against the three Isaac Sim variants (PD, constant-tension tendon, physical antiparallelogram tendon). Lower is better for all four metrics. Hatched bars marked "n/a" denote values that were not recorded in the source data. Time-trace plots underlying these aggregates are provided in @appendix:step_responses.],
