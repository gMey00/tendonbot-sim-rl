...
Devlin and Kudenko~@Devlin2012DynamicPBRS extended this result to _dynamic_ potentials that depend on time-varying state augmentations (e.g., persistent latch flags), showing that optimality is preserved even when the potential changes during an episode.

In practice, Popov _et al._~@Popov2017DataEfficientManipulation demonstrated the effectiveness of _composite shaping rewards_ for Lego brick stacking, where each reward component is gated by a task-phase predicate (e.g., transport reward activates only after a "grasped" condition is met).
