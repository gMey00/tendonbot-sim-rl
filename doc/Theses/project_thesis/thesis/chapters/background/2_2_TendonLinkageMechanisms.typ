...
==== Antagonistic arrangements
 Because cables can only pull, a single tendon cannot produce bidirectional joint torque. Joints are therefore actuated by _antagonistic_ tendon pairs (or groups), as shown in @fig:tendon_antagonistic. Differential tension produces a net joint torque, while simultaneous co-contraction modulates apparent joint stiffness without changing the joint angle~@Kobayashi1998NullSpaceStiffness@MuralidharanWenger2021. The resulting feasible torque–stiffness space is illustrated in @fig:torque_stiffness. For an $n$-#ac("DoF") system, Lee and Tsai~@TsaiLee1989 established that $n + 1$ cables are the minimum required for full force closure (bidirectional torque on all joints with positive cable tensions), while additional cables beyond $n + 1$ provide redundancy that can be exploited for stiffness modulation and tension optimization~@TsaiLee1989@Kobayashi1998NullSpaceStiffness.

#faps-figure(
  image("../../../../shared/figures/concept_antagonistic_joint.svg", width: 100%),
  caption: [Antagonistic tendon actuation at a single revolute joint shown in three configurations. Two cables $T_1$ (red) and $T_2$ (orange) are routed from proximal motors $M_1$ and $M_2$ on the fixed frame through a pulley of radius $r$ at the joint centre. Both cables attach to opposite sides of the pulley rim, with the colored markers indicating the attachment points. In the neutral configuration (centre), equal tensions produce zero net torque. Differential tension produces a net torque $tau = r(T_1 - T_2)$: when $T_1 < T_2$ (left) the joint deflects to negative angles, and when $T_1 > T_2$ (right) to positive angles. Simultaneous co-contraction ($T_1 + T_2$) modulates apparent joint stiffness $kappa$ independently of position, as shown quantitatively in @fig:torque_stiffness.],
  short-caption: [Antagonistic tendon actuation at a revolute joint],
) <fig:tendon_antagonistic>

#faps-figure(
  image("../../../../shared/figures/concept_torque_stiffness.svg", width: 85%),
  caption: [Feasible torque--stiffness space for an antagonistic tendon pair with tensions bounded by $0 <= T_i <= T_"max"$. The diamond-shaped region shows all achievable combinations of normalised net torque $tau slash tau_"max"$ and normalised apparent stiffness $kappa slash kappa_"max"$. The vertices correspond to the extremal tension combinations: maximum co-contraction at the top ($T_1 = T_2 = T_"max"$, maximum stiffness), zero tension at the bottom, and maximum unilateral torque on the sides. The dashed purple line indicates the null-space direction (co-contraction at zero torque), along which stiffness can be modulated without affecting the joint torque. The green lines illustrate the available stiffness range at two fixed torque values, demonstrating that stiffness modulation decreases as the demanded torque increases.],
...
==== Four-bar linkage fundamentals
 A planar four-bar linkage consists of four rigid bars connected by four revolute joints, forming a single closed loop with one degree of freedom~@McCarthy2010Linkages @Uicker2011Mechanisms. One bar is conventionally designated as the _frame_ (grounded link). The remaining links are the _input crank_, _coupler_, and _output rocker_. The Grashof condition classifies four-bar mechanisms by the range of motion of each link: if the sum of the shortest and longest link lengths does not exceed the sum of the other two, at least one link can rotate fully, yielding crank--rocker, double-crank, or double-rocker configurations~@Uicker2011Mechanisms. The configuration is fully determined by a single input angle through the _loop-closure equation_, which can be expressed in complex-number form as

$ bold(Z)_1 e^(i theta) + bold(Z)_2 e^(i beta) = bold(Z)_3 e^(i phi) + bold(Z)_4 $ <eq:bg_loop_closure>

where $bold(Z)_i$ denote the complex link vectors, $theta$ is the input angle, $phi$ the output angle, and $beta$ the coupler angle~@McCarthy2010Linkages. The _transmission angle_ (the angle between the coupler and the output link) characterizes force transmission quality. Values near $90 degree$ maximize force transfer, while values approaching $0 degree$ or $180 degree$ indicate near-singular configurations~@Uicker2011Mechanisms @HartenbergDenavit1964.

==== Antiparallelogram topology
 The antiparallelogram is a special case of the four-bar linkage in which the two side links (cranks) are of equal length and _cross_ each other, while the frame and coupler share a common length~@Dijksman1977Antiparallelogram @McCarthy2010Linkages. This crossed topology produces qualitatively different kinematics from the more common parallelogram arrangement. @fig:four_bar_types contrasts both topologies and visualises the swept workspace and #ac("ICR") behaviour.
...
#faps-figure(
  image("../../../../shared/figures/concept_four_bar_types.svg", width: 100%),
  caption: [Comparison of the two four-bar linkage topologies relevant to tendon-driven manipulators. Left: the standard parallelogram, where both side links (blue) remain parallel throughout motion and the instantaneous centre of rotation (#ac("ICR"), purple cross) lies at infinity, producing pure translational coupler motion. Right: the antiparallelogram, where the side links cross each other. The #ac("ICR") migrates along a centrode ellipse (purple dashed), creating a configuration-dependent lever arm. This nonlinear transmission ratio distinguishes the antiparallelogram from a simple revolute joint and requires either the physical linkage model or a disc approximation for torque computation. Multiple ghost poses at different input angles illustrate the swept workspace of each topology.],
  short-caption: [Parallelogram vs.\ antiparallelogram four-bar linkage topologies],
) <fig:four_bar_types>
...
#faps-figure(
  image("../../../../shared/figures/concept_cdpm.svg", width: 90%),
  caption: [Three-dimensional schematic of a cable-driven parallel mechanism with $m = 3$ cables and $n = 2$ rotational degrees of freedom, illustrating the general #ac("CDPM") concept underlying the wrist mechanism. Three cables (red, blue, orange) connect base frame anchors $b_i$ at $120 degree$ intervals on the fixed ring to platform anchors $p_i$ on the tilted movable ring. The platform is shown in a tilted configuration to visualise the cable arrangement. Each cable's unit direction vector $bold(u)_i$ and the resultant platform wrench $bold(w)$ are indicated. The static equilibrium is governed by the structure matrix: $bold(w) = bold(A)^top bold(t)$, where the one degree of redundancy ($m - n = 1$) enables tension optimisation under the positive-tension constraint $bold(t) >= 0$.],
  short-caption: [Cable-driven parallel mechanism concept with three cables],
) <fig:cdpm_concept>
...
#faps-figure(
  image("../../../../shared/figures/concept_tensegrity_classes.svg", width: 100%),
  caption: [Tensegrity structure classification according to Skelton and de Oliveira. Left: a class-1 tensegrity prism (3 struts, 9 cables) where no compression elements are in direct contact --- struts float within the continuous tension network of edge cables (solid red) and cross-face cables (dashed red). Right: a class-2 tensegrity module, where up to two struts may share a node (red dot). This topology corresponds to the antiparallelogram X-joint used in the elbow mechanism of the thesis robot, where the crossed struts form a planar linkage connected by cables and a coupler platform (green). The legend distinguishes compression elements (struts, grey), edge and cross-face tension elements (cables, red), antagonistic cables (orange), and the coupler link (green).],
