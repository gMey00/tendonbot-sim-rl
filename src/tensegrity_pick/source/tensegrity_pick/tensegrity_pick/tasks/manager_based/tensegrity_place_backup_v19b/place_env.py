...
from .place_scene_cfg import CONVEYOR_SURFACE_HEIGHT_M, CUBE_SIZE_M


EE_BODY_NAME = "tool_link_0"
GREEN_CUBE_KEY = "green_cube"

# Gripper geometry: grasp-centre offset from the EE body in local -z.
# See rewards.py for derivation from the Robotiq 2F-140 USD measurements.
GRASP_CENTER_LOCAL_Z = -0.1925

# Vertical offset: rewards target the cube TOP (not centre) so the
# gripper stops at the correct height instead of plunging into the belt.
GRASP_APPROACH_OFFSET_Z = CUBE_SIZE_M / 2.0  # 0.025 m

# Physics-based grasp detection thresholds
GRASP_PROXIMITY_THRESHOLD = 0.25          # metres – accounts for gripper length beyond EE mount
GRASP_LIFT_THRESHOLD = 0.03               # metres above belt surface – 5 mm lift from resting
