...
// State tracking for first-use expansion only
#let _acronym-state = state("acronyms", (:))
...
// _ac-marker is placed INSIDE the returned content block so it is emitted.
#let ac(key) = context {
  ...
  if key in used {
    ...
    _acronym-state.update(s => { s.insert(key, true); s })
...
// Reset first-use tracking (e.g. at document start)
#let ac-reset() = _acronym-state.update((:))
