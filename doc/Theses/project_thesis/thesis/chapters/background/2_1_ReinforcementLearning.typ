...
Further practical constraints include safety, limited real-world interaction budgets, and the need for robustness across object variations and perception nois. These characteristics influence algorithm choice, reward design, termination conditions, and the role of simulation during training.~@Kober2013Survey
...
Multi-step manipulation tasks such as pick-and-place naturally decompose into sequential phases (reach, grasp, transport, place), each requiring distinct behaviors. Ng _et al._~@Ng1999RewardShaping proved that adding a _potential-based_ shaping reward $F(s, a, s') = gamma Phi(s') - Phi(s)$ preserves all optimal policies of the original #ac("MDP"), which provides theoretical justification for adding phase-dependent intermediate rewards. Devlin and Kudenko~@Devlin2012DynamicPBRS extended this result to _dynamic_ potentials that depend on time-varying state augmentations (e.g., persistent latch flags), showing that optimality is preserved even when the potential changes during an episode. In practice, Popov _et al._~@Popov2017DataEfficientManipulation demonstrated the effectiveness of _composite shaping rewards_ for Lego brick stacking, where each reward component is gated by a task-phase predicate (e.g., transport reward activates only after a "grasped" condition is met). This gated activation pattern is equivalent to a two-state _reward machine_ in the formalism of Icarte _et al._~@Icarte2022RewardMachines, where finite-state automata define per-phase reward functions driven by propositional events. Benchmark environments such as robosuite~@Zhu2020robosuite implement this principle through explicit `staged_rewards()` methods with sequentially gated reach, grasp, lift, and hover components.
