...
from tensegrity_pick.robots.ur10e_robot_cfg import TARGET_LINK_NAME, CONTROLLED_JOINT_NAMES
...
class UR10eReachEnvCfg(ReachEnvCfg):
    ...
    def __post_init__(self):
        ...
        self.scene.robot.init_state.pos = (0.15, 0.0, UR10E_MOUNT_HEIGHT_M)
        ...
        # Narrow FK target sampling: after clamping limits to ±1.5 rad from
        # defaults (3 rad range per joint), margin=0.20 restricts targets to
        # the inner 60 % of each joint range (±0.90 rad from default).
        self.commands.ee_pose.joint_range_margin = 0.20
        ...
        # JointPositionToLimitsAction's 0.01 change → 8cm EE movement was too
        # coarse for PPO.  EMA smoothing provides gradual target changes.
        ...
        # ~30 cm, but UR10e's large workspace means average error starts at
        # ~50 cm.  std=0.5 provides useful gradient up to ~1.5 m.
        ...
        # multiplies defaults, so joints with default=0.0 (shoulder_pan,
        # wrist_3) never vary.  Offset-based adds a small random offset to
        # every joint, giving consistent starting diversity.
        ...
        # Clamp UR10e's wide ±2π (12.6 rad) joint limits to ±1.5 rad around
        # defaults.  This restricts both the FK target workspace and the action
        # mapping to a compact region, matching the difficulty level of the
        # tensegrity variant.  max_range=3.0 triggers for any joint with
        # range > 3 rad (all 6 UR10e joints qualify).
