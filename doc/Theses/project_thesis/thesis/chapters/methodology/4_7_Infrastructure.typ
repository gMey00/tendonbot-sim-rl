...
@fig:training_infrastructure summarises the four-layer software/hardware stack used for all training runs. From top to bottom: the *algorithm layer* hosts the #ac("PPO") agent in `skrl`~$gt.eq$~1.4.3 together with the sequential trainer and the logging back-ends. The *environment layer* exposes each task as a Gymnasium environment defined inside the `tensegrity_pick` Isaac~Lab extension and decomposed by the Isaac~Lab #ac("MDP") managers (observations, actions, rewards, terminations, curricula). The *simulation layer* is provided by Isaac~Sim~5.1.0 (#ac("USD") scene graph, `Cloner`-based scene replication) on top of the PhysX 5 backend (rigid bodies, articulations, fixed and spatial tendon constraints). The *hardware layer* is a single workstation with an NVIDIA RTX~A6000 #ac("GPU") (48~GB VRAM) running a dedicated conda environment.

#faps-figure(
  training-infrastructure-stack(),
  caption: [Training infrastructure stack used in this work. The #ac("PPO") learner (`skrl`~$gt.eq$~1.4.3) drives a vectorized Gymnasium environment that is realised through Isaac~Lab manager classes. Environment cloning, physics, and rendering execute on a single RTX~A6000 #ac("GPU") through the Isaac~Sim/PhysX backend. Custom diagnostic signals are streamed to TensorBoard and to a #acs("TUI")/web monitoring application.],
  short-caption: [Training infrastructure stack],
) <fig:training_infrastructure>

==== Logging and Monitoring <subsec:logging_monitoring>
`skrl`'s sequential trainer streams scalar reward components, episode lengths, and #ac("KL")/clipping diagnostics to TensorBoard. In addition, each task overrides the Isaac~Lab `RewardManager` to emit individually-weighted reward terms, which are essential for the per-term ablation tables in @ch:results. A custom monitoring application under `tools/monitor/` exposes the live training scalars both as a #ac("TUI") (`monitor.sh`) and as a web dashboard (`monitor.sh --web`). Both back-ends share the same parser and can be attached to a remote training run via #ac("SSH") port forwarding. More details on the monitoring tool are provided in @appendix:training_monitor.
